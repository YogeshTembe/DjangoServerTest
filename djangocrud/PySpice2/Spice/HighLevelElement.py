...
from ..Tools.StringTools import join_list, join_dict, str_spice, str_spice_list, join_list2, str_spice_list2
...
####################################################################################################
class SinusoidalMixin2(SourceMixinAbc):
    ...
    def __init__(self,
        ...
        self.frequency = frequency # Fixme: protect by setter?
        self.delay =delay
    ...
    def format_spice_parameters(self):
        ...
        sin_part = join_list2((self.offset, self.amplitude, self.frequency, self.delay, self.damping_factor))
        return join_list2((
            'DC {} AC {}'.format(*str_spice_list2(self.dc_offset, self.ac_magnitude)),
            'SIN({})'.format(sin_part),
        ))
...
class PulseMixin2(SourceMixinAbc):
    ...
    def __init__(self,
                 initial_value, pulsed_value,
                 pulse_width, period,
                 delay_time='0s', rise_time='0s', fall_time='0s',
                 phase=None,
                 dc_offset='0'):
        ...
        self.dc_offset =dc_offset # Fixme: -> SourceMixinAbc
        ...
        self.period = period # Fixme: protect by setter?
    ...
    def format_spice_parameters(self):
        ...
        # Fixme: to func?
        pulse_part=join_list2((self.initial_value, self.pulsed_value, self.delay_time,
                       self.rise_time, self.fall_time, self.pulse_width, self.period,
                       self.phase))
        return join_list2((
            'DC {}'.format(self.dc_offset),
            'PULSE({})'.format(pulse_part)))
...
####################################################################################################
class PieceWiseLinearMixin2(SourceMixinAbc):
...
####################################################################################################
class SinusoidalVoltageSource2(VoltageSource, VoltageSourceMixinAbc, SinusoidalMixin2):
    ...
    def __init__(self, netlist, name, node_plus, node_minus, *args, **kwargs):
        ...
        VoltageSource.__init__(self, netlist, name, node_plus, node_minus)
        SinusoidalMixin2.__init__(self, *args, **kwargs)

    ##############################################

    format_spice_parameters = SinusoidalMixin2.format_spice_parameters
...
class PulseVoltageSource2(VoltageSource, VoltageSourceMixinAbc, PulseMixin2):

    r"""This class implements a pulse waveform voltage source.

    See :class:`PulseMixin2` for documentation.
    ...
    def __init__(self, netlist, name, node_plus, node_minus, *args, **kwargs):
        ...
        VoltageSource.__init__(self, netlist, name, node_plus, node_minus)
        PulseMixin2.__init__(self, *args, **kwargs)

    ##############################################

    format_spice_parameters = PulseMixin2.format_spice_parameters
...
####################################################################################################
class PieceWiseLinearVoltageSource2(VoltageSource, VoltageSourceMixinAbc, PieceWiseLinearMixin2):
    ...
    def __init__(self, netlist, name, node_plus, node_minus, *args, **kwargs):
        ...
        VoltageSource.__init__(self, netlist, name, node_plus, node_minus)
        PieceWiseLinearMixin2.__init__(self, *args, **kwargs)

    ##############################################

    format_spice_parameters = PieceWiseLinearMixin2.format_spice_parameters
